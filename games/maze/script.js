...
function resetGame() {
  ...
  noteElement.innerHTML = `Boshlash uchun joystikni bosing!
          <p>Har bir to'pni markazga olib boring.</p>`;
  
  if (hardMode) {
    noteElement.innerHTML += `<p>Qora tuynuklarga e'tibor bering!</p>`;
  }
...
function main(timestamp) {
  ...
  try {
        ...
        if (hardMode) {
          holes.forEach((hole, hi) => {
            ...
            if (distance <= holeSize / 2) {
              ...
              throw Error("To'p qora tuynukka tushib ketdi");
            }
          });
        }
    ...
    if (
      ...
      noteElement.innerHTML = `Ofarin! Siz buni uddaladingiz!`;
      noteElement.style.opacity = 1;
      gameInProgress = false;
    } else {
      previousTimestamp = timestamp;
    ...
    if (error.message == "To'p qora tuynukka tushib ketdi") {
      noteElement.innerHTML = `To'p qora tuynukga tushib ketdi! O'yinni tiklash uchun bo'sh joy tugmasini bosing.`;
      noteElement.style.opacity = 1;
      gameInProgress = false;
    } else throw error;
  }
